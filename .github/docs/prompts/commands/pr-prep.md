...
- **Architecture & decisions** — ADR added where an alternative was rejected (or state
  ...
  regenerated via `make diagrams` · README ① Production ② Cost ③ Architecture still accurate
...
- Keep production figures out of the description: the deposition test applies to PR
  text too.
- Ticked packs must match the Issue and the task brief; flag any mismatch rather than
  guessing.
