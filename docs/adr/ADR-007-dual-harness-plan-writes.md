# ADR-0007 — Both harnesses may write plan artifacts; Claude Code's `/task-brief` runs unforked

- **Status:** Proposed
- **Date:** 2026-09-04
- **Deciders:** Manuel Reyes
- **Amends:** ADR-0004 (plan artifact ownership) — the "OpenCode owns the write" clause only
- **Related:** ADR-0005 (command shell trust boundary), ADR-0006 (subagent routing)
...
ADR-0004 assigned the plan-artifact write to OpenCode alone, on the grounds that OpenCode's permission model is a versioned glob that either matches or does not, while Claude Code's depends on `context: fork` + `agent:` — a path with open defects against
...
Code as a writer when what it should have rejected was **the forked path** as the mechanism. The distinction matters in practice: VS Code with Claude Code is where planning actually happens in this workflow, and ADR-0004 made the primary planning surface unable to persist its own primary artifact. Under it, `/task-brief` in VS Code
...
The boundary moves to `.claude/settings.json`, which is deterministic, versioned and reviewable — the same relocation ADR-0005 performed for command-template shell.
...
- **OpenCode:** unchanged from ADR-0004 — `"edit": { "*": "deny", ".github/plans/**": "allow" }` on `plan-cloud`.
...
Everything else in ADR-0004 stands unchanged: the path, the one-file-per-Issue naming, the `status: PROPOSAL` frontmatter, the prohibition on the agent ever writing
...
**Keep ADR-0004 as written; plan in VS Code, generate briefs in OpenCode.** Rejected: it splits one Gate 1 step across two applications and makes the artifact's location depend on which window happened to be open. The copy-paste handoff ADR-0002 existed to eliminate would return in a new form.
...
Reverse to ADR-0004's single-owner rule if the `settings.json` path scope proves
