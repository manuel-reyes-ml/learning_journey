...
def sort_senders(**emails):
    ...
    for email, count in emails.items():  #Use list comprehension and then 'key=' and lambda in sorted() function
        sorted_lst.append((count, email))
    
    sorted_lst = sorted(sorted_lst, reverse = True) #use return to sort at the same time
...
def main():
    ...
    for (count, email) in sort_senders(**emails):
        print(f"   {count}       {email}")
    print("\n")
